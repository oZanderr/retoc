...
fn resolve_script_import(package_cache: &FZenPackageContext, import: FPackageObjectIndex) -> anyhow::Result<ResolvedZenImport> {
    ...
    if package_cache.is_script_object_class(import) {
        return Ok(ResolvedZenImport {
            ...
            outer: Some(Box::new(resolved_outer_import)),
            object_name,
        });
    }
...
fn resolve_package_import_internal_new_common(package_cache: &FZenPackageContext, package_header: &FZenPackageHeader, import: FPackageObjectIndex) -> anyhow::Result<(FPackageId, Arc<FZenPackageHeader>, u64)> {
    ...
    if package_import.imported_package_index as usize >= package_header.imported_packages.len() {
        bail!(
            "Imported Package index out of bounds for import {}: package index is {}, but package has only {} imports total",
            import,
            package_import.imported_package_index,
            package_header.imported_packages.len()
        );
    }
    ...
    if package_import.imported_public_export_hash_index as usize >= package_header.imported_public_export_hashes.len() {
        bail!(
            "Imported Public Export Hash index out of bounds for import {}: hash index is {}, but package has only {} hashes total",
            import,
            package_import.imported_public_export_hash_index,
            package_header.imported_public_export_hashes.len()
        );
    }
...
fn resolve_package_export_internal(package_cache: &FZenPackageContext, package_header: &FZenPackageHeader, export: &FExportMapEntry) -> anyhow::Result<ResolvedZenImport> {
    ...
    if resolved_export_class.outer.is_none() || resolved_export_class.outer.as_ref().unwrap().outer.is_some() {
        bail!(
            "Resolved class of export {} of package {} has invalid class {} that does not have a package as it's outer",
            export_name,
            package_header.package_name(),
            resolved_export_class.object_name
        );
    }
...
fn copy_package_sections(builder: &mut LegacyAssetBuilder) -> anyhow::Result<()> {
    ...
    // Copy data resources
    builder.legacy_package.data_resources = builder
        .zen_package
        .bulk_data
        .iter()
        .map(|zen_bulk_data| {
...
fn build_export_map(builder: &mut LegacyAssetBuilder) -> anyhow::Result<()> {
    ...
    for export_index in 0..builder.zen_package.export_map.len() {
        ...
        let generate_public_hash = builder.zen_package.container_header_version >= EIoContainerHeaderVersion::LocalizedPackages && (object_flags & (EObjectFlags::Public as u32)) == 0 && zen_export.is_public_export();
...
fn resolve_export_dependencies_internal_dependency_bundles(builder: &mut LegacyAssetBuilder, export_dependencies: &mut [FStandaloneExportDependencies]) {
    for (extended_export_index, bundle_header) in builder.zen_package.dependency_bundle_headers.iter().enumerate() {
        ...
        let remap_zen_package_index = |x: FPackageIndex| -> FPackageIndex { if x.is_import() { FPackageIndex::create_import(*builder.original_import_order.get(&(x.to_import_index() as usize)).unwrap() as u32) } else { x } };
        ...
        dependencies.create_before_create = builder.zen_package.dependency_bundle_entries[first_dependency_index..last_create_before_create_index]
            .iter()
            .map(|x| x.local_import_or_export_index)
            .map(remap_zen_package_index)
            .collect();
        ...
        dependencies.serialize_before_create = builder.zen_package.dependency_bundle_entries[last_create_before_create_index..last_serialize_before_create_index]
            .iter()
            .map(|x| x.local_import_or_export_index)
            .map(remap_zen_package_index)
            .collect();
        ...
        dependencies.create_before_serialize = builder.zen_package.dependency_bundle_entries[last_serialize_before_create_index..last_create_before_serialize_index]
            .iter()
            .map(|x| x.local_import_or_export_index)
            .map(remap_zen_package_index)
            .collect();
        ...
        dependencies.serialize_before_serialize = builder.zen_package.dependency_bundle_entries[last_create_before_serialize_index..last_serialize_before_serialize_index]
            .iter()
            .map(|x| x.local_import_or_export_index)
            .map(remap_zen_package_index)
            .collect();
            ...
            for bundle_to_bundle_dependency_arc in &external_package_dependency.legacy_dependency_arcs {
                ...
                if let Err(resolution_error_message) = resolved_from_import_result {
                    info!(
                        ...
                        from_export_bundle_entry.local_export_index,
                        builder.package_id,
                        builder.zen_package.package_name(),
...
fn resolve_prestream_package_imports(builder: &mut LegacyAssetBuilder) -> anyhow::Result<()> {
    ...
    let all_imported_package_ids: HashSet<FPackageId> = builder
        .legacy_package
        .imports
        .iter()
    ...
    let prestream_package_ids: Vec<FPackageId> = builder.zen_package.imported_packages.iter().cloned().filter(|x| !all_imported_package_ids.contains(x)).collect();
        ...
        if current_legacy_asset_import_index >= builder.legacy_package.imports.len() {
            ...
            if !builder.has_failed_import_map_entries {
                info!(
                    ...
                    final_import_index,
                    import_map_size,
                    builder.legacy_package.summary.package_name.clone()
                );
            }
